...
serve(async (req: Request) => {
  ...
  // ── TURNSTILE_SECRET_KEY presence check ────────────────────────────────
  // In production this must always be set. In development it may be absent,
  // in which case only "dev-bypass-token" requests are allowed through.
  if (!TURNSTILE_SECRET_KEY) {
    console.warn("[auth-gate] TURNSTILE_SECRET_KEY not set — dev bypass mode active");
  }
  ...
  // DEV BYPASS: "dev-bypass-token" skips Cloudflare verification.
  // In production TURNSTILE_SECRET_KEY must be set and all tokens are verified.
  const isDevBypass = (turnstileToken as string) === "dev-bypass-token";

  if (isDevBypass) {
    console.warn("[auth-gate] DEV BYPASS — Turnstile verification skipped");
  } else {
    if (!TURNSTILE_SECRET_KEY) {
