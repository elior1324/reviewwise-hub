/**
 * receive-whatsapp-review — Edge Function
 *
 * Handles incoming WhatsApp customer feedback submitted via
 * the ReviewHub /wa/{flow_token} review page.
 *
 * This is an UNAUTHENTICATED endpoint — customers who click the
 * WhatsApp review link do NOT have a ReviewHub account.
 *
 * Security controls:
 *  - Validates flow_token exists and is active
 *  - Validates business exists
 *  - Rate limits: max 5 reviews per phone_hash per business
 *  - Content length limits
 *  - Reviews default to is_approved=false (business must approve)
 *
 * Future: When WhatsApp Cloud API is integrated, this same function
 * can receive webhook events from Meta with `whatsapp_message_id`.
 *
...
serve(async (req: Request) => {
  ...
  const {
    ...
    whatsapp_message_id,   // optional; from WhatsApp Cloud API webhook
  ...
  const { data: inserted, error: insertErr } = await admin
    ...
    .insert({
      ...
      source_label:        "WhatsApp Customer Feedback",
      source_url:          typeof source_url === "string" ? source_url : null,
      whatsapp_message_id: typeof whatsapp_message_id === "string" ? whatsapp_message_id : null,
      is_approved:         false,
  ...
  if (insertErr) {
    console.error("[receive-whatsapp-review] insert error:", insertErr);
