    ...
    <div className="min-h-screen bg-background noise-overlay" dir="rtl">
      ...
      <div className="container pt-20 pb-10">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsList className="rounded-xl border border-border/40 bg-card/50 p-3 h-auto flex-col items-stretch gap-3 w-full">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                ...
                <TabsTrigger value="trust-instagram" className="rounded-lg text-xs px-3 py-1.5 h-auto data-[state=active]:bg-primary data-[state=active]:text-primary-foreground gap-1">
                  <svg width={13} height={13} viewBox="0 0 24 24" className="ml-1">
                    <defs>
                      <linearGradient id="ig-tab-grad" x1="0%" y1="100%" x2="100%" y2="0%">
                        <stop offset="0%" stopColor="#F77737" />
                        <stop offset="50%" stopColor="#E1306C" />
                        <stop offset="100%" stopColor="#833AB4" />
                      </linearGradient>
                    </defs>
                    <path fill="url(#ig-tab-grad)" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
                  </svg>
                  ביקורות Instagram
                </TabsTrigger>
          ...
          {/* ── Instagram DM Review Collection Tab ───────────────────────── */}
          <TabsContent value="trust-instagram">
            <InstagramDMFlowPanel businessId={businessId} isDemo={isDemo} />
          </TabsContent>
...
// ── InstagramDMFlowPanel ──────────────────────────────────────────────────────
// Self-contained component for the "trust-instagram" tab.
// Mirrors WhatsAppFlowPanel — uses instagram_dm_review_flows table.
// ─────────────────────────────────────────────────────────────────────────────
...
const InstagramSvg = () => (
  <svg width={16} height={16} viewBox="0 0 24 24">
    <defs>
      <linearGradient id="ig-panel-grad" x1="0%" y1="100%" x2="100%" y2="0%">
        <stop offset="0%" stopColor="#F77737" />
        <stop offset="50%" stopColor="#E1306C" />
        <stop offset="100%" stopColor="#833AB4" />
      </linearGradient>
    </defs>
    <path fill="url(#ig-panel-grad)" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
  </svg>
);

interface PendingIGReview {
  id: string;
  author_name: string | null;
  rating: number | null;
  text: string;
...
const InstagramDMFlowPanel = ({
  ...
  const [linkCopied, setLinkCopied]         = useState(false);
  const [loadingFlow, setLoadingFlow]       = useState(false);
  const [pendingReviews, setPendingReviews] = useState<PendingIGReview[]>([]);
  const [approvingId, setApprovingId]       = useState<string | null>(null);
  const [rejectingId, setRejectingId]       = useState<string | null>(null);

  const BASE_URL = typeof window !== "undefined" ? window.location.origin : "https://reviewshub.info";
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .order("received_at", { ascending: false })
      .then(({ data }) => {
        if (data) setPendingReviews(data as PendingIGReview[]);
      });
  }, [businessId, isDemo]);

  const reviewLink = flowToken ? `${BASE_URL}/ig/${flowToken}` : null;
  ...
  if (isDemo) {
    return (
      <Card className="shadow-card bg-card max-w-xl">
        <CardContent className="py-8 text-center space-y-2">
          <InstagramSvg />
          <p className="text-sm font-semibold text-foreground">ביקורות Instagram DM</p>
          <p className="text-xs text-muted-foreground">
            אספו ביקורות מלקוחות ישירות דרך בוט Instagram — ללא צורך בחשבון ReviewHub מצד הלקוח.
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            זמין לעסקים רשומים בלבד.
          </p>
  ...
  return (
    <div className="space-y-6 max-w-2xl" dir="rtl">
      ...
      <Card className="shadow-card bg-card">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            <InstagramSvg /> ביקורות Instagram DM
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground leading-relaxed">
            שלחו ללקוחות שלכם קישור ייחודי דרך Instagram DM. הם ישאירו ביקורת ללא צורך בהרשמה.
            הביקורת תוצג בדף הפרופיל שלכם לאחר אישורכם.
          </p>
          ...
          <div
            className="flex items-start gap-2 text-[11px] text-muted-foreground/70 rounded-lg px-3 py-2.5"
            style={{ border: `1px solid ${IG_PINK}26`, background: `${IG_PINK}0a` }}
          >
            <InstagramSvg />
            <span>
              הקישור ייחודי לעסק שלכם. כל ביקורת שתתקבל תחכה לאישורכם לפני פרסום.
              שלחו אותו ללקוחות ב-Instagram DM לאחר שירות.
            </span>
          </div>
        </CardContent>
      </Card>
